  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "welcome",
              ...
              tags$br(),
              img(src='Instructions.png',height='550px')
      ...
      tabItem(tabName = "game",
              fluidRow(
                column(3,tags$div(id="inputsection"),
                       numericInput("pork","PORK [$2]",0),
                       htmlOutput("porknow"),
                       tags$br(),
                       numericInput("chicken","CHICKEN [$2]",0),
                       htmlOutput("chickennow"),
                       tags$br(),
                       numericInput("rice","RICE [$1]",0),
                       htmlOutput("ricenow"),
                       tags$br(),
                       numericInput("noodles","NOODLES [$1]",0),
                       htmlOutput("noodlesnow"),
                       tags$br(),
                       numericInput("vegetables","VEGETABLESset [$1.5]",0),
              ...
              fluidRow(
                box(
                  title = 'Message Box',solidHeader = TRUE,width =3,height = '460px',
                ...
                box(title = 'Recipes',solidHeader = TRUE,width = 5,status = 'primary',height = '460px',
                    fluidRow(
                      column(1.5),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$oldpasswordok,{
    ...
    print('checking now')
    correctpass<-getpassword(vals$playerid)
    checked<-(input$currentPassword == correctpass)
    print(checked)
    if (input$currentPassword==correctpass){
      #removeModal()
      print('True alr')
  ...
  observeEvent(input$confirm, {
    ...
    removeModal()
    new_ordered<-vals$orderplan$chicken[vals$round]+vals$orderplan$pork[vals$round]+vals$orderplan$rice[vals$round]+vals$orderplan$noodles[vals$round]+vals$orderplan$vegetables[vals$round]
    current_inven<-vals$stats[vals$stats$Day==7*vals$round-7,"Pork"]+vals$stats[vals$stats$Day==7*vals$round-7,"Rice"]+vals$stats[vals$stats$Day==7*vals$round-7,"Chicken"]+vals$stats[vals$stats$Day==7*vals$round-7,"Noodles"]+vals$stats[vals$stats$Day==7*vals$round-7,"Vegetables"]
    
    chiken_after<-vals$orderplan$chicken[vals$round]+vals$stats[vals$stats$Day==7*vals$round-7,"Chicken"]
    pork_after<-vals$orderplan$pork[vals$round]+vals$stats[vals$stats$Day==7*vals$round-7,"Pork"]
    rice_after<-vals$orderplan$rice[vals$round]+vals$stats[vals$stats$Day==7*vals$round-7,"Rice"]
    noodles_after<-vals$orderplan$noodles[vals$round]+vals$stats[vals$stats$Day==7*vals$round-7,"Noodles"]
    vegetables_after<-vals$orderplan$vegetables[vals$round]+vals$stats[vals$stats$Day==7*vals$round-7,"Vegetables"]
    
    if(new_ordered+current_inven>Inventory_limit){
      showModal(ExceedCapacityModal())
    }else if(chiken_after<0|pork_after<0|rice_after<0|noodles_after<0|vegetables_after<0){
